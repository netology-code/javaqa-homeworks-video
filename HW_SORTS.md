...
Перед вами класс, описывающий билет на самолёт (а точнее, предложение авиакомпании по перелёту):
```java
...
А ниже менеджер, который умеет хранить и искать эти билеты:
```java
...
Эту логику предстоит добавить вам.

Сперва сделайте так, что билеты по умолчанию сравниваются по цене.
...
Протестируйте работу метода `compareTo` в тестах.
...
Добавьте в менеджер метод `searchAndSortBy(String from, String to, Comparator<Ticket> comparator)`, который также бы искал билеты, но сортировал бы их уже по логике переданного параметром компаратора.
